...
def _(text):
    return translations.get(text, {}).get(current_lang, text)
...
# Use a significantly different video device number to avoid conflicts
def find_free_video_device():
    # Usamos un número mucho más alto para evitar cualquier conflicto
    return "/dev/video20"
...
def ensure_v4l2loopback_loaded():
    ...
    # Completamente descargar el módulo para asegurar una configuración limpia
    unload_v4l2loopback()
    time.sleep(2)
    
    # Usar video20 para evitar cualquier conflicto con otras cámaras
    video_device = "20"
    device_path = f"/dev/video{video_device}"
    
    try:
        # Cargar el módulo con un nombre único y canal específico
        result = subprocess.run(
            ["sudo", "modprobe", "v4l2loopback", 
             f"video_nr={video_device}", 
             "card_label=PhoneCam",  # Nombre muy distintivo
             "exclusive_caps=1"], 
        ...
        if result.returncode == 0 and os.path.exists(device_path):
            ...
            if result.returncode != 0:
                ...
                messagebox.showerror(_("Error"), _("Device not found") + f": {device_path}")
            status_label.config(text=_("Waiting for connection..."), fg=HIGHLIGHT_COLOR)
            return False
    except Exception as e:
        messagebox.showerror(_("Error"), str(e))
...
# Start ffmpeg and stream to the virtual device
def start_ffmpeg(ip, port):
    global ffmpeg_process
    output_device = find_free_video_device()

    # Siempre asegurarse de que el módulo esté correctamente cargado
    ...
    try:
        ...
        # Configuración mejorada para ffmpeg
        ffmpeg_process = subprocess.Popen([
            "ffmpeg", 
            "-re",  # Leer en tiempo real (mejor para cámaras virtuales)
            "-i", url,
            "-vf", "format=yuyv422",  # Asegura compatibilidad
            "-vcodec", "rawvideo",  # Salida sin compresión (requerido por v4l2loopback)
            "-pix_fmt", "yuyv422",  # Más estándar
            "-f", "v4l2",
            "-framerate", "25",
            "-video_size", "640x480",
            output_device
        ...
        device_info = ""
        try:
            v4l2_info = subprocess.check_output(["v4l2-ctl", "--list-devices"], text=True)
            device_info = f"\n\nDetected devices:\n{v4l2_info}"
        except:
            pass
        
        messagebox.showinfo(_("Camera connected"), 
                          _("Your phone camera is now available as 'PhoneCam' on") + 
                          f" {output_device}" + device_info)
    except Exception as e:
...
def load_module_advanced():
    ...
    device_nr.insert(0, "20")  # Cambiado a 20 para evitar conflictos
    device_nr.pack()
    ...
    card_label.insert(0, "PhoneCam")  # Nombre más distintivo
    card_label.pack()
    
    # Añadir opción para exclusive_caps
    ...
    exclusive_checkbox = tk.Checkbutton(adv_window, text="exclusive_caps=1", 
                                      ...
                                      selectcolor=BUTTON_BG, activebackground=DARK_BG)
    exclusive_checkbox.pack(pady=10)
...
def rebuild_gui():
    ...
    lang_menu = tk.Menu(menu_bar, tearoff=0, bg=BUTTON_BG, fg=LIGHT_TEXT, activebackground=BUTTON_ACTIVE)
    for code, label in SUPPORTED_LANGS.items():
        lang_menu.add_command(label=label, command=lambda c=code: change_language(c))
    menu_bar.add_cascade(label=_("Language"), menu=lang_menu)
...
def initialize_app():
    ...
    try:
        modinfo = subprocess.run(["modinfo", "v4l2loopback"], capture_output=True)
        if modinfo.returncode != 0:
            # El módulo no está instalado, mostraremos un mensaje después de iniciar la interfaz
            return
    except:
        return

    # First completamente unload the module to start fresh
    unload_v4l2loopback()
    time.sleep(2)

    # Then try to load the module with a very distinct device number and name
    try:
        # Try to load module at startup with proper parameters
        # Usamos un número alto (20) y un nombre muy distintivo
        subprocess.run(
            ["sudo", "modprobe", "v4l2loopback", "devices=1", "video_nr=20", 
             "card_label=PhoneCam", "exclusive_caps=1"], 
            capture_output=True, text=True, timeout=5
        )
        # Wait for device to be created
        time.sleep(3)
    except:
        pass  # If it fails, we'll handle it when the user tries to connect
